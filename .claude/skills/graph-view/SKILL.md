...
`edgeBudget(edges, nodeCount)` in edges.js: null (draw everything) while `edges ≤ 3.2/node`; beyond that, a Set of "src|tgt" keys = MAXIMUM spanning tree (Kruskal — guarantees every cluster stays connected, the "outer cluster connectivity" ask) + strongest extras to a SCALE-DECAYING allowance: `perNode = clamp(3.0 * sqrt(40/n), 1.6, 3.0)` — anchored at 3.0/node for 40 memories, sqrt-decaying to 1.6/node at scale (241 → ~386 shown). Budget skipped entirely when edges ≤ budget*1.1. paintLink skips non-budget edges at rest but ALWAYS shows ego/gathered/hovered edges AND connectors (`connectorKeys(edges)`: top-2 edges per pair of strong-graph components, alpha floor 0.32 — cluster bridges must never fade into invisible islands) — detail on demand, skeleton at rest (LGL-style, literature-backed). Threads also render zoom-constant: dash `[0.5,3]/zs` and `width/zs` with `zs = max(1, globalScale)^0.85`, so zoomed-in threads stay fine dotted lines instead of blobs.
...
- `gatherIds` = query highlightIds if non-null, else visibleIds when it's a strict subset AND the narrowing is POSITIVE (query/attribute chips/year/month — App passes `gatherActive`). Hiding a legend class is an exclusion: the remainder must stay put and just dim the hidden ones, never gather.
- Gathered targets PRESERVE the natural force-layout shape: subset compacted around its own centroid toward a COUNT-BASED target radius (`targetR = 34*sqrt(n)`, `shrink = min(0.8, targetR/spreadRMS)`) and recentered on the layout centroid — few matches huddle tight, many keep comfortable spread, independent of how scattered the matches originally were. (A phyllotaxis spiral was tried and scrapped: it scrambles neighbors, so every internal thread crosses the ball — chaos at 58 matches.) Overlap with dimmed "off" orbs is deliberately ignored, but the TARGETS get a one-time collision relaxation (40 pairwise-push iterations, radius r+7, in the gatherLayout memo) so actives never overlap each other on arrival — flight is unobstructed, the destination is collision-free ("collision reactivates at the center" for free). Gather easing 0.09/frame; wobble halves while gathered. A `gatherBudget` (edgeBudget over the gathered subgraph) disciplines internal threads of LARGE gathers exactly like the rest view; small gathers keep showing everything. Focus-repel distances are measured against BASE TARGETS (gathered or home) with R 120 while gathered, 170 otherwise — home-based distances are meaningless mid-gather.
