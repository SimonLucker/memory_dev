...
//   v3 tables → Supabase Postgres via PostgREST (loadSpace / upsert / remove)
//   v2 rows   → memories_v2 / cards / threads / profiles (person_id, id, data jsonb)
...
// ---- v3 relational tables (people, memories, moments, memory_people, questions, recaps)
// One generic contract: loadSpace / upsert / remove. Rows are plain objects
// shaped exactly like the table columns (see SCHEMA-v3.md), so no mapping.
const TABLES = ['people', 'memories', 'moments', 'memory_people', 'questions', 'recaps']
const json = { 'Content-Type': 'application/json' }

// → { people, memories, moments, links, questions, recaps } for one persona:
// memories they own or contribute to, with everything hanging off them, plus
// their contact list and every user. Remote: two PostgREST calls (the
// contributed ids first, because a filter on an embedded column would only
// narrow the embed, not the parent rows).
export async function loadSpace(personId) {
  const pid = encodeURIComponent(personId)
  if (!remote) {
    const r = await ok(await fetch(`/__db?space=${pid}`))
    return r.json()
  }
  const get = async q => (await ok(await fetch(`${SB_URL}/rest/v1/${q}`, { headers: sbHeaders }))).json()
  const ids = (await get(`memory_people?select=memory_id&person_id=eq.${pid}`)).map(r => r.memory_id)
  const where = ids.length ? `or=(owner_id.eq.${pid},id.in.(${ids.map(encodeURIComponent).join(',')}))` : `owner_id=eq.${pid}`
  const memories = await get(`memories?select=*,moments(*),memory_people(*),questions(*),recaps(*)&${where}&order=starts_at.desc`)
  const space = { people: [], memories: [], moments: [], links: [], questions: [], recaps: [] }
  for (const { moments, memory_people, questions, recaps, ...m } of memories) {
    space.memories.push(m)
    space.moments.push(...moments); space.links.push(...memory_people)
    space.questions.push(...questions); space.recaps.push(...recaps)
  }
  // Own contacts, every user, and whoever is linked to a visible memory (so a
  // contributor sees the owner's tagged people by name).
  const linked = [...new Set(space.links.map(l => l.person_id))].map(encodeURIComponent)
  space.people = await get(`people?or=(space_id.eq.${pid},is_user.eq.true${linked.length ? `,id.in.(${linked.join(',')})` : ''})`)
  return space
}

// Fire-and-forget writes: the store is already updated; a failure is logged,
// never surfaced. Batched: one call per table per save.
export function upsert(table, rows) {
  if (!TABLES.includes(table)) throw new Error(`unknown table ${table}`)
  if (!rows.length) return Promise.resolve()
  const p = remote
    ? withRetry(async () => ok(await fetch(`${SB_URL}/rest/v1/${table}`, {
      method: 'POST',
      headers: { ...sbHeaders, ...json, Prefer: 'resolution=merge-duplicates' },
      body: JSON.stringify(rows),
    })))
    : fetch('/__db', { method: 'POST', headers: json, body: JSON.stringify({ table, upsert: rows }) })
  return p.catch(e => console.warn(`upsert ${table} failed, kept in memory only`, e))
}

// id is a string, or {memory_id, person_id} for memory_people.
export function remove(table, id) {
  if (!TABLES.includes(table)) throw new Error(`unknown table ${table}`)
  const p = remote
    ? withRetry(async () => {
      const q = typeof id === 'object'
        ? Object.entries(id).map(([k, v]) => `${k}=eq.${encodeURIComponent(v)}`).join('&')
        : `id=eq.${encodeURIComponent(id)}`
      return ok(await fetch(`${SB_URL}/rest/v1/${table}?${q}`, { method: 'DELETE', headers: sbHeaders }))
    })
    : fetch('/__db', { method: 'POST', headers: json, body: JSON.stringify({ table, delete: id }) })
  return p.catch(e => console.warn(`remove ${table} failed`, e))
}

// ---- v2 jsonb rows (legacy surfaces and the capture engine still use these)
...
export async function loadMemories(personId) {
  ...
  const r = await ok(await fetch(
    `${SB_URL}/rest/v1/memories_v2?person_id=eq.${personId}&select=data&order=id.asc`,
    { headers: sbHeaders }))
...
export async function upsertMemory(personId, memory) {
  if (remote) {
    await ok(await fetch(`${SB_URL}/rest/v1/memories_v2`, {
      method: 'POST',
...
export async function removeMemory(personId, id) {
  if (remote) {
    await ok(await fetch(`${SB_URL}/rest/v1/memories_v2?person_id=eq.${personId}&id=eq.${id}`, {
      method: 'DELETE',
      headers: sbHeaders,
    }))
  } else {
...
// `cards_v2` table (person_id, id, data jsonb). Dev: the /__cards middleware,
...
export async function loadCards(personId) {
  try {
    if (remote) {
      const r = await ok(await fetch(
        `${SB_URL}/rest/v1/cards_v2?person_id=eq.${personId}&select=data&order=id.asc`,
        { headers: sbHeaders }))
      return (await r.json()).map(row => row.data)
    }
...
export async function upsertCard(personId, card) {
  if (remote) {
    await ok(await fetch(`${SB_URL}/rest/v1/cards_v2`, {
      method: 'POST',
      headers: { ...sbHeaders, 'Content-Type': 'application/json', Prefer: 'resolution=merge-duplicates' },
...
export async function removeCard(personId, id) {
  if (remote) {
    await ok(await fetch(`${SB_URL}/rest/v1/cards_v2?person_id=eq.${personId}&id=eq.${id}`, {
      method: 'DELETE',
      headers: sbHeaders,
    }))
  } else {
