# Memmory Design Foundation v2.5

**v2.5 additions (July 28, after first deployed build review, decided by Simon A):** the Vault view switch is no longer a large segmented pill. It is a round 30px control in the top RIGHT corner (the counterpart of the avatar circle top left) showing a glyph for the current view (list lines in List, constellation in Cortex). Tapping it folds it out into the List | Cortex pill anchored top right (600ms, pushes content below down in flow); choosing a segment switches the view and folds the pill back into the circle · in Cortex mode the cortexDeep gradient fills the ENTIRE pane including behind the chrome row, no dawn band above the field · heat pill: year labels always fit fully inside the bar; a single-year span shows up to 4 month labels instead of a year · dragging along the heat pill shows a floating readout pill above the finger with the date under it (month + year on multi-year spans, day + month on single-year), fading 200ms on release · profiles without a precomputed layout must spread organically, never collapse into one ball (collision radius plus degree-scaled link strength).
